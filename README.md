...
<p align="center">
  <img src="assets/banner.png" alt="Antigravity Hub Banner" width="100%" />
</p>
...
**`antigravity-hub`** is the master barista's hopper and switchboard:
- **The Central Bean Catalog (`~/.gemini/catalog/skills/`)**: Stores all 100+ available skills offline in a pristine pantry.
- **The Persona Pod Selector (`bin/agy-persona`)**: Switches your active environment in milliseconds. Switching to `stitch` loads only design tools; switching to `architect` loads PRD and red-teaming skills; switching to `core` strips everything back down to a razor-sharp minimalist espresso shot (< 5 skills).
- **The Upstream Quality Inspector (`bin/agy-plugins`)**: Continuously audits upstream GitHub repositories and Git remotes, letting you know when upstream bean batches receive fresh releases.
...
```
┌──────────────────────────────────────────────────────────────────────────────────────────────────┐
│                                ANTIGRAVITY HUB DISCOVERY TAXONOMY                                │
├───────────────────────────────┬───────────────────────────────┬──────────────────────────────────┤
...
├───────────────────────────────┼───────────────────────────────┼──────────────────────────────────┤
│ • 108+ Offline Skills         │ • Curated symlinks only       │ • Plugin toggles (enabled: T/F)  │
...
flowchart TD
    subgraph Catalog["Central Pantry (~/.gemini/catalog/skills/)"]
        AllSkills["108+ Cataloged Skills\n(gcloud, stitch-*, waf-*, etc.)"]
    end

    subgraph Switcher["Persona Switcher (agy-persona)"]
        CorePod["core.json\n(Minimalist Baseline)"]
...
Antigravity Hub ships with 6 curated persona profiles in [`personas/`](personas/):

| Persona | Purpose | Enabled Plugins | Curated Active Skills |
| :--- | :--- | :--- | :--- |
| **`core`** | **Minimalist Baseline**. Fast everyday coding, scripts, and dotfiles maintenance. | `antigravity-hub` | `git-delivery`, `agy-customizations`, `antigravity-guide`, `graphify` *(< 5 skills!)* |
| **`architect`** | **Bean-to-Cup SDLC**. Socratic grilling, PRDs, domain modeling, and red-team audits. | `antigravity-hub`, `bean-to-cup` | `ideator`, `grill`, `write-prd`, `domain-modeling`, `feature`, `kanban`, `audit-code`, `research`, `sync`, `visual-dashboard` |
| **`fullstack`** | **Web Application Engineering**. React, Vite, Tailwind, and Chrome DevTools debugging. | `antigravity-hub`, `modern-web-guidance-plugin`, `chrome-devtools-plugin` | `react-vite-dashboard`, `shadcn-ui`, `a11y-debugging`, `chrome-devtools`, `chrome-extensions`, `debug-optimize-lcp`, `memory-leak-debugging` |
| **`stitch`** | **UI/UX Design Studio**. Google Stitch design generation, screen extraction, and Remotion video walkthroughs. | `antigravity-hub`, `stitch-build`, `stitch-design`, `stitch-utilities` | `design-md`, `enhance-prompt`, `stitch-code-to-design`, `stitch-extract-design-md`, `stitch-generate-design`, `stitch-loop`, `stitch-react-components`, `remotion`, `taste-design` |
| **`gcp-sre`** | **Cloud Infrastructure & Reliability**. GCP services, databases, networking, and chaos mitigation. | `antigravity-hub` | `gcloud`, `cloud-run-basics`, `cloud-sql-basics`, `alloydb-basics`, `bigquery-basics`, `gke-basics`, `google-cloud-waf-*`, `iam-recommendations-fetcher`, `chaos-mitigation` |
| **`adk-dev`** | **AI Agent Platform Engineering**. Google ADK, Agent Platform, and Antigravity SDK. | `antigravity-hub`, `google-antigravity-sdk` | `google-agents-cli-*`, `agent-platform-eval-flywheel`, `agent-platform-model-registry`, `agent-platform-prompt-management`, `gemini-agents-api` |

---

## 🔍 The Upstream Quality & Update Inspector
...
**`agy-plugins`** solves this by maintaining an automated upstream audit engine:
...
Run `agy-plugins update` to update.
```
...
Clone or locate the repository under `~/workspace/antigravity-hub` and run the automated setup script:
...
3. **Installs Global CLI Tools**: Symlinks `agy-hub`, `agy-persona`, and `agy-plugins` into `~/.local/bin/`.
4. **Registers Plugin & Hook**: Links `antigravity-hub` into `~/.gemini/config/plugins/` so its `SessionStart` hook is active.
...
# Active Plugins (1): antigravity-hub
```
...
#    Plugins enabled : stitch-utilities, antigravity-hub, stitch-build, stitch-design
...
### Persona Management (`agy-persona`)
| Command | Description |
| :--- | :--- |
| `agy-persona list` | List all available personas with description, plugins, and skill counts |
| `agy-persona switch <name>` | Switch active persona (`core`, `architect`, `fullstack`, `stitch`, `gcp-sre`, `adk-dev`) |
| `agy-persona current` | Display active persona, loaded skills list, and enabled plugins |
| `agy-persona reset` | Return to minimal baseline (`core` profile) |
| `agy-persona load <name>` | Temporarily load an individual skill or plugin into the active session |
| `agy-persona unload <name>` | Temporarily unload a skill or plugin |

### Plugin Version Audit (`agy-plugins`)
| Command | Description |
| :--- | :--- |
| `agy-plugins check` | Check installed plugins against upstream GitHub releases and Git remotes |
| `agy-plugins check --markdown` | Format results as Markdown (used by the launch hook) |
| `agy-plugins check --json` | Machine-readable JSON output for automations |
| `agy-plugins list` | List local versions and configurations of all installed plugins |
| `agy-plugins update [all\|name]` | Pull latest upstream commits or release updates |

### Unified Dispatcher (`agy-hub`)
```bash
agy-hub persona switch architect
agy-hub plugins check
agy-hub plugins update all
```
...
Antigravity Hub includes unit tests verifying persona JSON schema integrity, dispatcher execution, update audits, and launch hook speed:
...
Antigravity Hub works seamlessly alongside the rest of the Autonomous Barista Swarm:
...
*   **Antigravity Hub**: The master switchboard ensuring each barista has only the tools they need for the perfect pour.
